module.exports.config = {
	...
	hasPermssion: 2,
	credits: "Dũng UwU, hotfix by Thọ, fix coins thành đô by Hung2z",
	...
	switch(args[0]) {
		...
		if(args[2] == "all") return api.sendMessage(`Bạn đã giảm toàn bộ tiền của bản thân`, threadID, async () => {
		 ...
		 if(coins > balance) return api.sendMessage("Số tiền bạn giảm lớn hơn số tiền hiện có",threadID,messageID)
	       	else return api.sendMessage(`Đã giảm ${coins} đô của bản thân`, threadID, async () => {
	    ...
	    api.sendMessage({ body: `Đã giảm ${args[1+ nameL]} đô của ` + event.mentions[userID].replace(/@/g, "")}, threadID, async () => {
